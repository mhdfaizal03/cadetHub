...
class MarkAttendanceHistoryScreen extends StatefulWidget {
  const MarkAttendanceHistoryScreen({super.key});

  @override
  State<MarkAttendanceHistoryScreen> createState() =>
      _MarkAttendanceHistoryScreenState();
}

class _MarkAttendanceHistoryScreenState
    extends State<MarkAttendanceHistoryScreen>
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        final allItems = snapshot.data!.docs
            ...
            .where((p) {
              // Filter for PAST parades for history
              try {
        ...
        if (yearsToShow.length == 1) {
          return _buildParadeListView(
            ...
            "No past ${yearsToShow.first} parades",
          );
        }
  ...
  Widget _buildCampList(UserModel officer) {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        final allItems = snapshot.data!.docs
            .map(
              (d) => CampModel.fromMap(d.data() as Map<String, dynamic>, d.id),
            )
            .where((c) {
              // Filter for PAST camps
              try {
                final eDate = DateTime.parse(c.endDate); // Use End Date
        ...
        allItems.sort((a, b) => b.startDate.compareTo(a.startDate));
        ...
        if (yearsToShow.length == 1) {
          return _buildCampListView(
            getForYear(yearsToShow.first),
            "No past ${yearsToShow.first} camps",
          );
        }
  ...
  Widget _buildExamList(UserModel officer) {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        final allItems = snapshot.data!.docs
            .map(
              (d) => ExamModel.fromMap(d.data() as Map<String, dynamic>, d.id),
            )
            .where((e) {
              // Filter for PAST Exams
              try {
                // Use end date if available, or start date
                // Assuming exams have end date mostly
                final eDate = DateFormat('MMM d, yyyy').parse(e.endDate);
                final nDate = DateTime(eDate.year, eDate.month, eDate.day);
                return nDate.isBefore(today);
              } catch (err) {
                return false;
        ...
        if (yearsToShow.length == 1) {
          return _buildExamListView(
            getForYear(yearsToShow.first),
            "No past ${yearsToShow.first} exams",
          );
        }
